...
const CaseDocumentsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.fileType)) {
                error["fileType"] = `File Type field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.storagePath)) {
                error["storagePath"] = `Storage Path field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            caseNo: _entity?.caseNo?._id,user: _entity?.user,fileName: _entity?.fileName,fileType: _entity?.fileType,storagePath: _entity?.storagePath,extractedContent: _entity?.extractedContent,uploadTimestamp: _entity?.uploadTimestamp,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="user" className="w-full mb-3 p-inputtext-sm" value={_entity?.user} onChange={(e) => setValByKey("user", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="fileType" className="w-full mb-3 p-inputtext-sm" value={_entity?.fileType} onChange={(e) => setValByKey("fileType", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="storagePath" className="w-full mb-3 p-inputtext-sm" value={_entity?.storagePath} onChange={(e) => setValByKey("storagePath", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="uploadTimestamp"  value={_entity?.uploadTimestamp ? new Date(_entity?.uploadTimestamp) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("uploadTimestamp", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
